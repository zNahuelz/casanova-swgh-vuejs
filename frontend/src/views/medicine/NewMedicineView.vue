...
//TODO: Make the form load to get the IGV from the database config.
//Im tired '-'
...
const selectedPresentationId = ref(4); //Change this to 5 for test. SupId for 3.
const selectedSupplierId = ref(3);
...
function onPricesChanges() {
  ...
  if (igvStatus.value) {
    const subtotal = sell / (1 + IGV_VALUE);
...
<template>
  <main class="flex flex-col items-center pt-5">
    <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm min-w-150">
      ...
      <ScanBarcodeForm v-if="!barcodeLocked" @barcodeGenerated="handleBarcodeGeneration"></ScanBarcodeForm>
      ...
      <Form class="space-y-3 mt-5" v-if="barcodeLocked" :validation-schema="medicineSchema" @submit="onSubmit">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div class="flex items-center ps-4 border border-gray-300 rounded-lg h-10 bg-gray-50">
              ...
              <input id="salable" type="checkbox" name="salable" v-model="salable"
        ...
        <div class="flex justify-center mt-5" v-if="barcodeLocked">
