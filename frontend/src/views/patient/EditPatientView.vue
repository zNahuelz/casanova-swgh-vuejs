...
const schema = yup.object().shape({
  ...
  address: yup
      ...
      .matches(/^\S.*$/, 'La dirección materno no debe comenzar con espacios.')
...
async function loadPatient(id) {
  ...
  try {
    ...
    const maternalSurname = patient.value.maternal_surname;
    const paternalSurname = patient.value.paternal_surname;
    if (!maternalSurname || maternalSurname.trim().length === 0) {
      patient.value.maternal_surname = '';
    }
    if (!paternalSurname || paternalSurname.trim().length === 0) {
      patient.value.paternal_surname = '';
    }
    console.log(patient.value);
  } catch (err) {
    loadError.value = true;
    Swal.fire(EM.ERROR_TAG, EM.PATIENT_NOT_FOUND, 'error').then((r) => {
...
async function onSubmit(values) {
  ...
  try {
    const response = await PatientService.update(patient.value.id, payload);
...
<template>
  <main class="flex flex-col items-center pt-5">
    <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm  min-w-150">
      ...
      <Form v-if="!isLoading && !loadError" ref="patientForm" v-slot="{validate}" :validation-schema="schema"
            class="space-y-3"
            @submit="onSubmit">
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-900 ">Nombre</label>
          <Field id="name" :disabled="submitting" :model-value="patient.name" :validate-on-input="true"
        ...
        <div>
          ...
          <Field id="paternal_surname" :disabled="submitting" :model-value="patient.paternal_surname"
        ...
        <div>
          ...
          <Field id="maternal_surname" :disabled="submitting" :model-value="patient.maternal_surname"
                 :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
        ...
        <div>
          ...
          <Field id="birth_date" :disabled="submitting" :model-value="patient.birth_date" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                 name="birth_date"
                 type="date"/>
          <ErrorMessage class="mt-1 text-sm text-red-600 dark:text-red-500 font-medium"
        ...
        <div>
          ...
          <Field id="dni" :disabled="submitting" :model-value="patient.dni"
                 :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                 name="dni"
                 type="text"/>
        ...
        <div>
          ...
          <Field id="email" :disabled="submitting" :model-value="patient.email" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
                 name="email"
                 type="email"/>
        ...
        <div>
          ...
          <Field id="phone" :disabled="submitting" :model-value="patient.phone" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                 name="phone"
                 type="text"/>
        ...
        <div>
          ...
          <Field id="address" :disabled="submitting" :model-value="patient.address" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
                 name="address"
                 type="text"/>
          <ErrorMessage class="mt-1 text-sm text-red-600 dark:text-red-500 font-medium"
        ...
        <div class="flex justify-center mt-5">
          <div class="inline-flex rounded-md shadow-xs" role="group">
            ...
            <button :disabled="submitting || patient.dni === '00000000'"
