  ...
  <main class="flex flex-col items-center pt-5">
    <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm  min-w-150">
      ...
      <div v-if="isLoading" class="container mt-5 mb-5 flex flex-col items-center">
        ...
        <h1 class="mt-5 text-2xl font-light">Cargando paciente...</h1>
      </div>
      <div v-if="!isLoading && !loadError" class="space-y-3">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-3">
              <div>
                ...
                <div class="mt-1 text-sm text-gray-900">
                  {{ formatAsDatetime(patient?.created_at) }}
                </div>
              </div>
              <div>
                ...
                <div class="mt-1 text-sm text-gray-900">
                  {{
                    !patient?.created_by?.display_name ? 'INSERTADO EN BASE DE DATOS' : patient.created_by.display_name
                  }}
                </div>
              </div>
            </div>
            <div class="space-y-3">
              <div>
                ...
                <div class="mt-1 text-sm text-gray-900">
                  {{ formatAsDatetime(patient?.updated_at) }}
                </div>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-500">Modificado por</label>
                <div class="mt-1 text-sm text-gray-900">
                  {{ !patient?.updated_by?.display_name ? '---' : patient.updated_by.display_name }}
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div>
            ...
            <input :value="patient?.paternal_surname + ' ' + patient?.maternal_surname"
          ...
          <div>
            ...
            <input :value="patient?.birth_date"
                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-800 focus:border-green-800 w-full appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                   disabled
                   type="text"/>
          </div>
          <div>
            <label class="block mb-1 text-sm font-medium text-gray-900 ">Edad Actual</label>
            <input
                :value="calculateAge(patient?.birth_date) + ' AÑOS'"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-800 focus:border-green-800 w-full"
                disabled
                type="text"/>
          </div>
          ...
          <div>
            ...
            >
              ...
              {{ patient?.appointments?.length > 0 ? 'HISTORIAL DE CITAS' : 'SIN CITAS RESERVADAS' }}
            </button>
          </div>
