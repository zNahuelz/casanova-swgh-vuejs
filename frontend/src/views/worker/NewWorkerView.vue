...
onMounted(() => {
  document.title = 'ALTERNATIVA CASANOVA - NUEVO TRABAJADOR'
});
</script>
...
<template>
  <main class="flex flex-col items-center pt-5">
    <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm  min-w-150">
      <h5 class="mb-2 text-2xl font-bold tracking-tight text-black text-center">NUEVO TRABAJADOR</h5>
      <Form ref="workerForm" v-slot="{validate}" :validation-schema="schema" class="space-y-3" @submit="onSubmit">
        <div>
        ...
        <div>
          ...
          <Field id="hiring_date" :disabled="submitting" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
                 name="hiring_date"
                 type="date"/>
          <ErrorMessage class="mt-1 text-sm text-red-600 dark:text-red-500 font-medium"
        ...
        <div>
          ...
          <Field id="position" v-model="selectedPosition" :disabled="submitting" :validate-on-input="true" as="select"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
                 name="position"
                 @change="validate">
