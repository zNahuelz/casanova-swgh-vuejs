...
import {WORKER_SEARCH_MODES as WSM} from "@/utils/constants.js";
...
const dynamicSchema = computed(() => {
  ...
  } else if (searchMode.value === 'position') {
    keywordValidation = keywordValidation.min(3, 'El cargo debe tener al menos 3 carácteres');
  }
...
async function loadWorkers(filters = {}) {
  ...
  try {
    ...
    const response = await WorkerService.get(filters, pagination);
...
function onSubmit(values) {
  ...
  if (values.searchMode === 'position') {
    filters = {position: keyword};
    currentPage.value = 1;
    loadWorkers(filters);
  }
}
...
function goToDetails(id) {
  router.push({name: 'worker-detail', params: {id}});
}

onMounted(() => {
  document.title = 'ALTERNATIVA CASANOVA - LISTADO DE TRABAJADORES'
  loadWorkers();
});
</script>
  ...
  <main class="flex flex-col items-center pt-5 relative">
    <div class="container px-12 mx-auto">
      <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm w-full">
        <h5 class="mb-2 text-2xl font-bold tracking-tight text-black text-start" v-if="!isLoading">LISTADO DE TRABAJADORES</h5>
        ...
        <div v-if="isLoading" class="container mt-5 mb-5 flex flex-col items-center">
          ...
          <h1 class="mt-5 text-2xl font-light">Cargando trabajadores...</h1>
        </div>
          ...
          <Form v-slot="{ validate, meta }" :validation-schema="dynamicSchema" @submit="onSubmit">
            <div class="flex">
              <Field id="searchMode" v-model="searchMode" as="select"
                     ...
                     name="searchMode"
                     @change="validate">
                <option v-for="sm in WSM" :key="sm.value" :value="sm.value">{{ sm.label }}</option>
              </Field>
        ...
        <div v-if="!isLoading && !loadError" class="container mt-5 mb-5">
          <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
              ...
              <tbody>
              <tr
                  v-for="w in workers" :key="w.id" class="bg-white border-b border-gray-200 hover:bg-gray-50">
                ...
                <td class="px-6 py-2 font-medium text-gray-900">
                  {{ w.paternal_surname + ' ' + w.maternal_surname }}
                </td>
                <td class="px-6 py-2">
                  {{ w.dni }}
                </td>
            ...
            <span
                class="text-sm font-normal text-gray-500  mb-4 md:mb-0 block w-full md:inline md:w-auto">{{workers.length}} de un total de <span
                class="font-semibold text-gray-900 ">{{ totalItems }}</span> trabajadores</span>
        ...
        <div v-if="loadError" class="container mt-5 mb-5 flex flex-col items-center space-y-5">
          ...
          <h1 class="text-2xl font-light">Oops! No se encontraron trabajadores con los parametros ingresados. Intente
