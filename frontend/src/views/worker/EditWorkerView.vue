...
import {ERROR_MESSAGES as EM, SUCCESS_MESSAGES as SM, WORKER_POSITIONS as WP} from "@/utils/constants.js";
...
const selectedPosition = ref('SECRETARIA');
const validPositions = ref(WP.map(e => e.value));
...
const schema = yup.object().shape({
  ...
  maternal_surname: yup
      ...
      .test(
          ...
          value => {
            ...
            return /^[a-zA-ZáéíóúüñÁÉÍÓÚÜÑ\s]{2,30}$/.test(value);
          }),
  dni: yup
      .string()
  ...
  hiring_date: yup
      ...
      .max(new Date(), 'La fecha debe estar en el pasado o ser el día actual.'),
  position: yup
      .string()
      .required('Debe seleccionar un cargo.')
      .oneOf(validPositions.value, 'Cargo no valido.')
});
...
async function loadWorker(id) {
  ...
  try {
    ...
    const rawDate = worker.value.worker?.hiring_date;
    formattedHiringDate.value = rawDate ? dayjs.utc(rawDate).format('YYYY-MM-DD') : '';
    const maternalSurname = worker.value.worker?.maternal_surname;
    const paternalSurname = worker.value.worker?.paternal_surname;
    if (!maternalSurname || maternalSurname.trim().length === 0) {
      worker.value.worker.maternal_surname = '';
    }
    if (!paternalSurname || paternalSurname.trim().length === 0) {
      worker.value.worker.paternal_surname = '';
    }
  } catch (err) {
...
async function onSubmit(values) {
  ...
  try {
    const response = await WorkerService.update(worker.value.worker.id, payload);
...
onMounted(() => {
  document.title = 'ALTERNATIVA CASANOVA - GESTIONAR TRABAJADOR'
...
<template>
  <main class="flex flex-col items-center pt-5">
    <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm  min-w-150">
      ...
      <Form v-if="!isLoading && !loadError" ref="workerForm" v-slot="{validate}" :validation-schema="schema"
            class="space-y-3"
            @submit="onSubmit">
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-900 ">Nombre</label>
          <Field id="name" :disabled="submitting" :model-value="worker?.worker?.name" :validate-on-input="true"
        ...
        <div>
          ...
          <Field id="paternal_surname" :disabled="submitting" :model-value="worker?.worker?.paternal_surname"
        ...
        <div>
          ...
          <Field id="maternal_surname" :disabled="submitting" :model-value="worker?.worker?.maternal_surname"
                 :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
        ...
        <div>
          ...
          <Field id="dni" :disabled="submitting" :model-value="worker?.worker?.dni" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                 name="dni"
                 type="text"/>
        ...
        <div>
          ...
          <Field id="email" :disabled="submitting" :model-value="worker?.worker?.email" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
                 name="email"
                 type="email"/>
        ...
        <div>
          ...
          <Field id="phone" :disabled="submitting" :model-value="worker?.worker?.phone" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                 name="phone"
                 type="text"/>
        ...
        <div>
          ...
          <Field id="address" :disabled="submitting" :model-value="worker?.worker?.address" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
                 name="address"
                 type="text"/>
          <ErrorMessage class="mt-1 text-sm text-red-600 dark:text-red-500 font-medium"
        ...
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-900 ">Fecha de Contratación</label>
          <Field id="hiring_date" :disabled="submitting" :model-value="formattedHiringDate" :validate-on-input="true"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
                 name="hiring_date"
                 type="date"/>
          <ErrorMessage class="mt-1 text-sm text-red-600 dark:text-red-500 font-medium"
                        name="hiring_date"></ErrorMessage>
        </div>

        <div>
          <label class="block mb-1 text-sm font-medium text-gray-900 ">Cargo</label>
          <Field id="position" v-model="selectedPosition" :disabled="submitting" :model-value="worker?.worker?.position"
                 :validate-on-input="true"
                 as="select"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  focus:ring-green-800 focus:border-green-800 w-full"
                 name="position"
                 @change="validate">
            <option v-for="e in WP" :key="e.value" :value="e.value">{{ e.label }}</option>
          </Field>
          <ErrorMessage class="mt-1 text-sm text-red-600 dark:text-red-500 font-medium"
                        name="position"></ErrorMessage>
        </div>
