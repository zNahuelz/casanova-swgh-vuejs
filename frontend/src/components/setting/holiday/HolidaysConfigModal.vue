...
import {formatAsDate, reloadOnDismiss, reloadPage} from "@/utils/helpers.js";
import NewHolidayModal from "@/components/setting/holiday/NewHolidayModal.vue";
import Swal from "sweetalert2";
import {SUCCESS_MESSAGES as SM} from "@/utils/constants.js";
import EditHolidayModal from "@/components/setting/holiday/EditHolidayModal.vue";
...
async function loadHolidays(filters = {}) {
  try {
    isLoading.value = true;
    holidays.value = await HolidayService.get();
...
function deleteHoliday(holiday) {
  Swal.fire({
    ...
    html: `¿Está seguro de eliminar el siguiente feriado? <br> NOMBRE: ${holiday.name} - FECHA: ${formatAsDate(holiday.date)} <br> De continuar se habilitará la reserva de citas en ese día.`,
    icon: 'question',
...
function showHelp() {
  Swal.fire(
      'Información',
      `<span class="font-light">La configuración de feriados funciona de la siguiente manera:</span>
  <ul class="list-disc pl-5 mt-2 text-left font-light">
    <li><strong class="font-bold text-black-900">Días bloqueados:</strong> Durante todo el feriado, <strong class="text-red-800 font-bold">no se pueden reservar citas.</strong></li>
    <li><strong class="font-bold text-black-900">Feriados recurrentes:</strong> Se repiten automáticamente cada año, sin importar que la fecha almacenada sea de un año pasado (ej: Navidad, Año Nuevo, Fiestas Patrias).</li>
    <li><strong class="font-bold text-black-900">Feriados no recurrentes:</strong> Solo son válidos el año específico definido (ej: un feriado puntual como un evento único).</li>
  </ul>`,
      'info'
  );
}

function handleNewHolidayModal() {
  showNewHolidayModal.value = !showNewHolidayModal.value;
}

function handleEditHolidayModal(holiday) {
  selectedHoliday.value = holiday;
  showEditHolidayModal.value = !showEditHolidayModal.value;
}
...
<template>
  <div id="voucher-config-modal"
    ...
    <div class="relative bg-white rounded-lg shadow-lg w-full max-w-5xl">
      ...
      <div class="p-6 space-y-6">
        <div v-if="isLoading" class="container mt-5 mb-5 flex flex-col items-center">
          ...
          <h1 class="mt-5 text-2xl font-light">Cargando feriados...</h1>
        </div>
          ...
          <div class="max-h-[300px] overflow-y-scroll">
            <table class="min-w-full bg-white border border-gray-200 rounded-lg mt-3">
              ...
              <tr>
                ...
                <th class="px-4 py-2 border-b text-left text-sm font-medium text-gray-700 w-50">NOMBRE</th>
                <th class="px-4 py-2 border-b text-left text-sm font-medium text-gray-700">FECHA</th>
                <th class="px-4 py-2 border-b text-left text-sm font-medium text-gray-700">RECURRENTE</th>
              ...
              <tr v-for="h in holidays" :key="h.id" class="hover:bg-gray-50">
                <td class="px-4 py-2 border-b text-sm text-gray-600">{{ h.id }}</td>
                <td class="px-4 py-2 border-b text-sm text-gray-600">{{ h.name }}</td>
                <td class="px-4 py-2 border-b text-sm text-gray-600">{{ formatAsDate(h.date) }}</td>
                <td :class="{'text-green-800 font-bold': h.is_recurring, 'text-red-800 font-bold': !h.is_recurring}"
                    class="px-4 py-2 border-b text-sm text-gray-600">
                  {{ h.is_recurring ? 'SI' : 'NO' }}
                </td>
                <td class="px-4 py-2 border-b text-sm text-gray-600">
                  <ul>
                    <li class="font-bold text-blue-800"
                        @click="handleEditHolidayModal(h)"
                    >EDITAR
                    </li>
        ...
        <div v-if="loadError" class="container mt-5 mb-5 flex flex-col items-center space-y-5">
          ...
          <h1 class="text-2xl font-light">Oops! No se encontraron feriados con los parametros ingresados. Intente
            nuevamente o registre algunos.</h1>
    ...
    <NewHolidayModal v-if="showNewHolidayModal" :onClose="handleNewHolidayModal"></NewHolidayModal>
    <EditHolidayModal v-if="showEditHolidayModal" :holiday="selectedHoliday"
                      :onClose="() => { showEditHolidayModal = false; }"></EditHolidayModal>
  </div>
</template>