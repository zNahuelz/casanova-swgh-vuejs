...
const {onClose, app} = defineProps(['onClose', 'app']);
const submitting = ref(false);
const appPriceForm = ref();

const schema = yup.object({
  price: yup
      .string()
      .matches(/^(1000(\.0{1,2})?|[1-9]\d{0,2}(\.\d{1,2})?)$/, 'Debe ser un número positivo entre 1 y 5000 con hasta 2 decimales')
      .test('is-valid-range', 'El costo debe estar entre 1 y 5000', (value) => {
        const num = parseFloat(value);
        return !isNaN(num) && num >= 1 && num <= 5000;
      })
      .required('El costo de la cita es obligatorio.'),
...
async function onSubmit(values) {
  ...
  try {
    const payload = {
      ...
      appPrice: parseFloat(values.price),
      description: values.description.length >= 1 ? values.description : null,
    };
    const response = await SettingService.updateAppointmentPrice(payload);
...
<template>
  <div id="edit-app-price" class="fixed inset-0 z-50 flex items-center justify-center bg-white/10 backdrop-blur-sm"
       tabindex="-1">
    <div class="relative bg-white rounded-lg shadow-lg w-100 max-w-md">
      <div
          class="flex items-center justify-between p-4 md:p-5 border-b rounded-t  border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">
          Modificar variable: {{ app.key }}
        </h3>
        ...
        <div class="flex p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50" role="alert">
          ...
          <div>
            ...
            <ul class="mt-1.5 list-disc list-inside">
              <li>La actualización del costo de la cita tomará efecto inmediato posterior a su cambio.
              </li>
              <li>Los valores en comprobantes de pago antiguos no se verán afectados.</li>
              <li>Los pagos y reembolsos pendientes no se verán afectados.</li>
              <li>No es necesario que modifique la descripción.</li>
            </ul>
          </div>
        </div>
        <Form ref="appPriceForm" :initial-values="{
            price: parseFloat(app.value) || 120,
