...
const {onClose, serie} = defineProps(['onClose', 'serie']);
...
const schema = yup.object({
  serie: yup
      ...
      .matches(
          /^[BF](00[1-9]|0[1-9][0-9]|[1-9][0-9]{2})$/,
          'La serie debe tener el formato Bxxx ó Fxxx.'
      ),
...
async function onSubmit(values) {
  ...
  try {
    ...
    const response = await SettingService.updateVoucherSeries(payload);
...
onMounted(() => {
  if (serie.serie.includes('B')) {
    selectedVoucherType.value = VST[0].value;
  } else {
    selectedVoucherType.value = VST[1].value;
  }
});
...
<template>
  <div id="edit-series" class="fixed inset-0 z-50 flex items-center justify-center bg-white/10 backdrop-blur-sm"
       tabindex="-1">
    <div class="relative bg-white rounded-lg shadow-lg w-100 max-w-md">
      <div
          class="flex items-center justify-between p-4 md:p-5 border-b rounded-t  border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">
          Comprobante de Pago: Editar Serie - {{ serie.serie }}
        </h3>
      ...
      <div class="p-6 space-y-6 items-center flex flex-col">
        ...
        <div v-if="submitting" class="container mt-5 mb-5 flex flex-col items-center">
          ...
          <h1 class="mt-5 text-2xl font-light">Actualizando...</h1>
        </div>

        <Form v-if="!submitting" ref="seriesForm" :initial-values="{
          serie: serie.serie,
          next_correlative: serie.next_correlative || '1',
        }" :validation-schema="schema" class="grid gap-6"
              @submit="onSubmit"
        >
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-900">Serie (1-999)</label>
            <Field id="serie" :validate-on-input="true" class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
          ...
          <div>
            ...
            <Field id="type" v-model="selectedVoucherType" :validate-on-input="true" as="select" class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                    focus:ring-green-600 focus:border-green-600 block w-full p-2.5"
                   disabled
                   name="type">
          ...
          <div class="inline-flex rounded-md shadow-xs" role="group">
            <button :disabled="submitting"
                    class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-white border-s border-t border-b border-gray-200 rounded-s-lg hover:bg-gray-100 hover:text-red-700 focus:z-10 focus:ring-2 focus:ring-red-700 focus:text-red-700 disabled:bg-gray-200 disabled:cursor-not-allowed"
                    type="button"
                    @click="onClose">
