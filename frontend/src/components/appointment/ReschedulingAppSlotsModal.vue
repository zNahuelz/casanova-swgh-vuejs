...
const {
  ...
  patientInfo,
  oldAppointment
} = defineProps(['onClose', 'selectedDay', 'doctorInfo', 'patientInfo', 'oldAppointment']);
const onCloseEnabled = ref(true);
const submitting = ref(false);

const isRemote = ref(oldAppointment.is_remote);
const router = useRouter();

async function rescheduleAppointment(selectedDay, s) {
  Swal.fire({
    title: SM.CONFIRM_OPERATION,
    html: `¿Desea reprogramar la siguiente cita? <br>
    ...
    <span><span class="font-bold">NUEVO DÍA:</span> ${selectedDay?.weekdayName?.toUpperCase()}</span>
    <span><span class="font-bold">FECHA INICIAL:</span> ${oldAppointment.date} => <span class="font-bold">NUEVA FECHA: </span>${selectedDay.dateKey}</span>
    <span><span class="font-bold">HORA INICIAL:</span> ${oldAppointment.time} => <span class="font-bold">NUEVA HORA: </span>${s.start_time}</span>
    ...
    <div class="relative bg-white rounded-lg shadow-lg w-full max-w-3xl">
      ...
      <div class="pb-4 mt-4">
            ...
            >
              ...
              <td class="px-6 py-2">
                <button
                    ...
                    type="button"
                    @click="rescheduleAppointment(selectedDay,s)">
                  Reprogramar
                </button>
              </td>
            </tr>
        ...
        <div v-if="submitting" class="container mt-5 mb-5 flex flex-col items-center">
          ...
          <h1 class="mt-5 text-2xl font-light">Reprogramando cita...</h1>
        </div>
      </div>
    </div>
  </div>
