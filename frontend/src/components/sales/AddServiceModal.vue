...
import {SUPPLIER_SEARCH_MODES as SSM, TREATMENT_SEARCH_MODES as TSM} from "@/utils/constants.js";
import {ErrorMessage, Field, Form} from "vee-validate";

const emit = defineEmits(['addServiceToList']);
...
const dynamicSchema = computed(() => {
  let keywordValidation = yup.string().required('Debe ingresar un críterio de busqueda.');
...
async function loadTreatments(filters = {}) {
  ...
  try {
    const response = await TreatmentService.get(filters);
...
<template>
  <div id="search-services" tabindex="-1"
    ...
    <div class="relative bg-white rounded-lg shadow-lg w-auto">
      <div
        ...
        <button type="button" @click="onClose" :disabled="submitting || isLoading"
      ...
      <div class="p-6 space-y-6 items-center flex flex-col">
        <div class="container mt-3 mb-3">
          ...
          <table class="w-full text-sm text-left rtl:text-right text-gray-500 mt-5" v-if="!isLoading && treatments.length >= 1">
            ...
            <tbody>
            <tr
                class="bg-white border-b border-gray-200 hover:bg-gray-50" v-for="t in treatments" :key="t.id">
              <th scope="row" class="px-6 py-2 whitespace-nowrap">
                {{ t.id }}
              </th>
              <td class="px-6 py-2 font-medium text-gray-900 w-50 text-wrap">
                {{ t.name }}
              </td>
              <td class="px-6 py-2 font-medium text-gray-900 w-50 text-wrap">
                {{ t.description }}
              </td>
              <td class="px-6 py-2 font-medium text-gray-900 text-green-700 font-bold">
                S./ {{ t.price}}
              </td>
              <td class="px-6 py-3 flex justify-center items-center">
                <div class="inline-flex rounded-md shadow-xs" role="group">
                  <button type="button" @click="() => {emit('addServiceToList',t)}"
          ...
          <div class="container mt-5 mb-5 flex flex-col items-center" v-if="isLoading">
            ...
            <h1 class="mt-5 text-2xl font-light">Buscando tratamientos...</h1>
          </div>
          <div class="container mt-5  flex flex-col items-center"
            ...
            <h1 class="mt-5 text-lg font-light text-center">No se encontraron tratamientos con el criterio
              ingresado.</h1>
          </div>
        </div>
      </div>
