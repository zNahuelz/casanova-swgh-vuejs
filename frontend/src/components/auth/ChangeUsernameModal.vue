...
async function onSubmit(values) {
  ...
  try {
    const response = await UserService.changeUsername(values);
    ...
    } else if (err.aux === 'INVALID_PASSWORD') {
      ...
      <div class="p-6 space-y-6 items-center flex flex-col">
        <Form ref="presentationForm" :validation-schema="schema" class="grid gap-6" @submit="onSubmit">
          <div>
            ...
            <Field id="username" v-model="usernameValue" :disabled="submitting" :validate-on-input="true" class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
